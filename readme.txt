...
3 ) rendering - render game to screen

Both event processing & rendering inside game loop(control the lifetime of application). 
As long as this one continues to iterate, the application will stay alive.
Inside the game loop, we process the user events from the window, then update game logic
and finally rendering the results on screen.
